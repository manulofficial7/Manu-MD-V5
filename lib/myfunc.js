...
exports.buffergif = async (image) => {
	...
	const filename = `${Math.random().toString(36)}`
			await fs.writeFileSync(`./ManuMedia/trash/${filename}.gif`, image)
					 child_process.exec(
								`ffmpeg -i ./ManuMedia/trash/${filename}.gif -movflags faststart -pix_fmt yuv420p -vf "scale=trunc(iw/2)*2:trunc(ih/2)*2" ./ManuMedia/trash/${filename}.mp4`
										) 
  await sleepy(4000)
  
	var buffer5  =  await  fs.readFileSync(`./ManuMedia/trash/${filename}.mp4`)
	Promise.all([unlink(`./ManuMedia/video/${filename}.mp4`), unlink(`./ManuMedia/gif/${filename}.gif`)])
	return buffer5
				   }
...
exports.GIFBufferToVideoBuffer = async (image) => {
    ...
    var buffer5 = await fs.readFileSync(`./ManuMedia/trash/${filename}.mp4`)
    Promise.all([unlink(`./ManuMedia/trash/${filename}.mp4`), unlink(`./ManuMedia/trash/${filename}.gif`)])
    return buffer5
}